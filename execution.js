...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      async function getTextFromJson(paraId) {
        try {
          const response = await fetch(paraId + ".json");
          if (!response.ok) {
            throw new Error("Network response was not ok");
          }
          const jsonData = await response.json();

          const words = jsonData.map((obj) => obj.word);
      ...
      function main(paraId) {
        ...
        fetch(paraId + ".json")
          ...
          .then((data) => {
            ...
            console.log(
              "This is to check whether wordMap is filled properly. ",
              wordMap.size
            );
            // let map = new Map();
          })
      ...
      function executeRemainingCode() {
        const inputText = document.getElementById("textInput").value;
        console.log(
          "I am in execute remaining code size of wordmap is",
          wordMap.size
        );
